...
class cniveles_add extends cniveles {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "nivelesview.php")
	...
	function AddRow($rsold = NULL) {
		...
		} elseif (intval($this->idNivel->CurrentValue) < -2) {
			$this->setFailureMessage($Language->Phrase("UserLevelIDIncorrect"));
		} elseif (intval($this->idNivel->CurrentValue) == 0 && !ew_SameText($this->denominacion->CurrentValue, "Default")) {
			$this->setFailureMessage($Language->Phrase("UserLevelDefaultName"));
		} elseif (intval($this->idNivel->CurrentValue) == -1 && !ew_SameText($this->denominacion->CurrentValue, "Administrator")) {
			$this->setFailureMessage($Language->Phrase("UserLevelAdministratorName"));
		} elseif (intval($this->idNivel->CurrentValue) == -2 && !ew_SameText($this->denominacion->CurrentValue, "Anonymous")) {
			$this->setFailureMessage($Language->Phrase("UserLevelAnonymousName"));
		} elseif (intval($this->idNivel->CurrentValue) > 0 && in_array(strtolower(trim($this->denominacion->CurrentValue)), array("anonymous", "administrator", "default"))) {
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->idNivel->setDbValue($conn->Insert_ID());
				$rsnew['idNivel'] = $this->idNivel->DbValue;
			}
		} else {
...
var CurrentForm = fnivelesadd = new ew_Form("fnivelesadd", "add");
...
?>
<form name="fnivelesadd" id="fnivelesadd" class="<?php echo $niveles_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($niveles_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $niveles_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $niveles_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
