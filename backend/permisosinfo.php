...
class cpermisos extends cTable {
	...
	function __construct() {
		...
		// idNivel
		$this->idNivel = new cField('permisos', 'permisos', 'x_idNivel', 'idNivel', '`idNivel`', '`idNivel`', 3, -1, FALSE, '`idNivel`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tabla
		$this->tabla = new cField('permisos', 'permisos', 'x_tabla', 'tabla', '`tabla`', '`tabla`', 200, -1, FALSE, '`tabla`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->tabla->Sortable = TRUE; // Allow sort
		$this->fields['tabla'] = &$this->tabla;

		// permisos
		$this->permisos = new cField('permisos', 'permisos', 'x_permisos', 'permisos', '`permisos`', '`permisos`', 3, -1, FALSE, '`permisos`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->permisos->Sortable = TRUE; // Allow sort
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('idNivel', $rs))
				ew_AddFilter($where, ew_QuotedName('idNivel', $this->DBID) . '=' . ew_QuotedValue($rs['idNivel'], $this->idNivel->FldDataType, $this->DBID));
			if (array_key_exists('tabla', $rs))
				ew_AddFilter($where, ew_QuotedName('tabla', $this->DBID) . '=' . ew_QuotedValue($rs['tabla'], $this->tabla->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@idNivel@", ew_AdjustSql($this->idNivel->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		$sKeyFilter = str_replace("@tabla@", ew_AdjustSql($this->tabla->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "idNivel:" . ew_VarToJson($this->idNivel->CurrentValue, "number", "'");
		$json .= ",tabla:" . ew_VarToJson($this->tabla->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["idNivel"]))
				$arKey[] = ew_StripSlashes($_POST["idNivel"]);
			elseif (isset($_GET["idNivel"]))
				$arKey[] = ew_StripSlashes($_GET["idNivel"]);
			else
				$arKeys = NULL; // Do not setup
			if ($isPost && isset($_POST["tabla"]))
				$arKey[] = ew_StripSlashes($_POST["tabla"]);
			elseif (isset($_GET["tabla"]))
				$arKey[] = ew_StripSlashes($_GET["tabla"]);
			else
				$arKeys = NULL; // Do not setup
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->permisos->EditValue = $this->permisos->CurrentValue;
