...
$paginas_estaticas_add = NULL; // Initialize page object first

class cpaginas_estaticas_add extends cpaginas_estaticas {
	...
	// Page object name
	var $PageObjName = 'paginas_estaticas_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "paginas_estaticasview.php")
	...
	function LoadDefaultValues() {
		...
		$this->imagenPrincipal->Upload->DbValue = NULL;
		$this->imagenPrincipal->OldValue = $this->imagenPrincipal->Upload->DbValue;
		$this->imagenPrincipal->CurrentValue = NULL; // Clear file related field
		$this->contenido->CurrentValue = NULL;
		$this->contenido->OldValue = $this->contenido->CurrentValue;
		$this->fechaCreacion->CurrentValue = ew_CurrentDateTime();
		$this->usuarioCreacion->CurrentValue = CurrentUserName();
		$this->fechaModificacion->CurrentValue = NULL;
		$this->fechaModificacion->OldValue = $this->fechaModificacion->CurrentValue;
		$this->usuarioModificacion->CurrentValue = NULL;
		$this->usuarioModificacion->OldValue = $this->usuarioModificacion->CurrentValue;
	}
	...
	function RenderRow() {
		...
		$this->usuarioModificacion->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->imagenPrincipal);
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->imagenPrincipal->Visible && !$this->imagenPrincipal->Upload->KeepFile) {
			$this->imagenPrincipal->Upload->DbValue = ""; // No need to delete old file
		...
		// fechaCreacion
		$this->fechaCreacion->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fechaCreacion->CurrentValue, 0), NULL, FALSE);

		// usuarioCreacion
		$this->usuarioCreacion->SetDbValueDef($rsnew, $this->usuarioCreacion->CurrentValue, NULL, FALSE);
...
// Create page object
if (!isset($paginas_estaticas_add)) $paginas_estaticas_add = new cpaginas_estaticas_add();

// Page init
$paginas_estaticas_add->Page_Init();

// Page main
$paginas_estaticas_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$paginas_estaticas_add->Page_Render();
?>
...
var CurrentForm = fpaginas_estaticasadd = new ew_Form("fpaginas_estaticasadd", "add");

// Validate form
fpaginas_estaticasadd.Validate = function() {
...
fpaginas_estaticasadd.Form_CustomValidate = 
...
fpaginas_estaticasadd.ValidateRequired = true;
<?php } else { ?>
fpaginas_estaticasadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fpaginas_estaticasadd.Lists["x_idMenu"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"_menu"};
...
<?php if (!$paginas_estaticas_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $paginas_estaticas_add->ShowPageHeader(); ?>
<?php
$paginas_estaticas_add->ShowMessage();
?>
<form name="fpaginas_estaticasadd" id="fpaginas_estaticasadd" class="<?php echo $paginas_estaticas_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($paginas_estaticas_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $paginas_estaticas_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateEditor("fpaginas_estaticasadd", "x_contenido", 35, 4, <?php echo ($paginas_estaticas->contenido->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $paginas_estaticas_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fpaginas_estaticasadd.Init();
</script>
<?php
$paginas_estaticas_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$paginas_estaticas_add->Page_Terminate();
?>
