...
class cniveles_edit extends cniveles {
	...
	function Page_Main() {
		...
		switch ($this->CurrentAction) {
			...
			Case "U": // Update
				...
				if (ew_GetPageName($sReturnUrl) == "niveleslist.php")
	...
	function RenderRow() {
		...
		$this->denominacion->ViewCustomAttributes = "";
			...
			if (in_array($this->idNivel->CurrentValue, array(-2,-1,0))) $this->denominacion->ReadOnly = TRUE;
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, $this->AddMasterUrl("niveleslist.php"), "", $this->TableVar, TRUE);
		$PageId = "edit";
...
var CurrentForm = fnivelesedit = new ew_Form("fnivelesedit", "edit");
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $niveles->denominacion->FldCaption(), $niveles->denominacion->ReqErrMsg)) ?>");
...
?>
<form name="fnivelesedit" id="fnivelesedit" class="<?php echo $niveles_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($niveles_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $niveles_edit->Token ?>">
<?php } ?>
...
<?php if ($niveles->idNivel->Visible) { // idNivel ?>
	<div id="r_idNivel" class="form-group">
		<label id="elh_niveles_idNivel" class="col-sm-2 control-label ewLabel"><?php echo $niveles->idNivel->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $niveles->idNivel->CellAttributes() ?>>
<span id="el_niveles_idNivel">
<span<?php echo $niveles->idNivel->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $niveles->idNivel->EditValue ?></p></span>
</span>
<input type="hidden" data-table="niveles" data-field="x_idNivel" name="x_idNivel" id="x_idNivel" value="<?php echo ew_HtmlEncode($niveles->idNivel->CurrentValue) ?>">
<?php echo $niveles->idNivel->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($niveles->denominacion->Visible) { // denominacion ?>
	<div id="r_denominacion" class="form-group">
		<label id="elh_niveles_denominacion" for="x_denominacion" class="col-sm-2 control-label ewLabel"><?php echo $niveles->denominacion->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $niveles->denominacion->CellAttributes() ?>>
<span id="el_niveles_denominacion">
<input type="text" data-table="niveles" data-field="x_denominacion" name="x_denominacion" id="x_denominacion" size="30" maxlength="255" placeholder="<?php echo ew_HtmlEncode($niveles->denominacion->getPlaceHolder()) ?>" value="<?php echo $niveles->denominacion->EditValue ?>"<?php echo $niveles->denominacion->EditAttributes() ?>>
</span>
<?php echo $niveles->denominacion->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$niveles_edit->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $niveles_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
